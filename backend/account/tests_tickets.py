...
class EmployeeTicketsAPITests(TestCase):
    ...
    def test_employee_ticket_comment_can_include_links_and_files(self):
        ticket = EmployeeTicket.objects.create(
            employee=self.employee,
            title="Proof update",
            description="",
        ...
        self.client.force_authenticate(user=self.emp_user)
        upload = SimpleUploadedFile("proof.txt", b"done proof", content_type="text/plain")
        res = self.client.post(
            f"/api/employee-tickets/{ticket.id}/comments/",
            data={
                "text": "completed with proof",
                "links": json.dumps([{"url": "example.com/result", "label": "Result"}]),
                "files": upload,
            },
            format="multipart",
        )
        self.assertEqual(res.status_code, 201)
        body = res.json()
        self.assertEqual(body.get("links", [{}])[0].get("url"), "https://example.com/result")
        self.assertEqual(len(body.get("attachments", [])), 1)

        res2 = self.client.get(f"/api/employee-tickets/{ticket.id}/comments/")
        self.assertEqual(res2.status_code, 200)
        latest = res2.json()[0]
        self.assertTrue(latest.get("links"))
        self.assertTrue(latest.get("attachments"))
