...
def _user_display_name(user):
    ...
    return full or getattr(user, "email", "") or getattr(user, "username", "") or "Someone"
...
def _employee_display_name(employee):
    ...
    return _user_display_name(user) or getattr(employee, "employee_id", "") or str(getattr(employee, "id", ""))


def _normalize_mention_label(value):
    return re.sub(r"[^a-z0-9]+", "", str(value or "").strip().lower())


def _mentioned_labels(text):
    return {_normalize_mention_label(m) for m in re.findall(r"@([A-Za-z0-9_.-]+)", text or "") if m}


def _extract_ticket_links(data):
    raw = data.get("links", data.get("link", data.get("url", "")))
    values = []
...
def _ticket_uploads(request):
    files = []
    for key in ("files", "attachments", "attachment", "file"):
        try:
    ...
    for file_obj in files:
        marker = (getattr(file_obj, "name", ""), getattr(file_obj, "size", None))
        if marker in seen:
            continue
        seen.add(marker)
...
def _save_ticket_attachments(ticket, request, comment=None):
    created = []
    for file_obj in _ticket_uploads(request):
        created.append(
...
def _notify_ticket_watchers(ticket, actor, kind, message, comment=None, mention_label=""):
    actor_id = getattr(actor, "id", None)
    recipients = {}

    def add_user(user):
        if user and getattr(user, "id", None) and user.id != actor_id:
            recipients[f"user:{user.id}"] = {"recipient_user": user, "recipient_employee": getattr(user, "employee_profile", None)}

    def add_employee(employee):
        if employee and getattr(employee, "id", None):
            user = getattr(employee, "user", None)
            if not user or getattr(user, "id", None) == actor_id:
                return
            recipients[f"employee:{employee.id}"] = {"recipient_user": user, "recipient_employee": employee}

    add_employee(getattr(ticket, "employee", None))
    add_employee(getattr(ticket, "assigned_to", None))

    labels = _mentioned_labels(message)
    if "admin" in labels:
        for user in User.objects.filter(Q(is_staff=True) | Q(is_superuser=True) | Q(is_admin=True)).distinct():
            add_user(user)

    if labels:
        for employee in EmployeeProfile.objects.select_related("user").all():
            choices = {
                _normalize_mention_label(_employee_display_name(employee)),
                _normalize_mention_label(getattr(employee, "employee_id", "")),
                _normalize_mention_label(getattr(getattr(employee, "user", None), "email", "")),
            }
            if choices.intersection(labels):
    ...
    title = getattr(ticket, "title", "") or getattr(ticket, "ticket_number", "") or f"Ticket #{ticket.pk}"
    if kind == "status":
        notification_title = f"{actor_name} marked a ticket as Done"
        notification_message = f"{actor_name} marked \"{title}\" as done. Please check it once."
    elif kind == "mention":
        notification_title = f"{actor_name} mentioned you on a ticket"
        notification_message = message
    else:
        notification_title = f"{actor_name} commented on a ticket"
        notification_message = message

    EmployeeTicketNotification.objects.bulk_create(
        [
            EmployeeTicketNotification(
                ticket=ticket,
                comment=comment,
...
def _notification_payload(notification):
    ...
    return {
        ...
        "ticket": getattr(ticket, "id", None),
        "ticket_title": getattr(ticket, "title", ""),
        "ticket_number": getattr(ticket, "ticket_number", ""),
        "actor_name": _user_display_name(actor) if actor else "",
...
class EmployeeTicketsAPI(DebugForce200Mixin, APIView):
    ...
    def post(self, request):
        ...
        with transaction.atomic():
            ...
            _save_ticket_attachments(obj, request)
        ...
        if not _is_admin(request.user):
            ...
            if status_value not in (None, "", "null"):
                ...
                if mapped_status in {"open", "in_progress", "resolved", "closed"}:
                    status_changed_to_done = obj.status != mapped_status and mapped_status == "resolved"
            ...
            if isinstance(text, str) and text.strip():
                comment = EmployeeTicketComment.objects.create(
                    ...
                    text=text.strip(),
                    links=_extract_ticket_links(request.data),
                )
                _save_ticket_attachments(obj, request, comment=comment)
                if _mentioned_labels(text):
                    _notify_ticket_watchers(obj, request.user, "mention", text.strip(), comment=comment)
                elif status_changed_to_done:
                    _notify_ticket_watchers(obj, request.user, "status", text.strip(), comment=comment)
            elif _ticket_uploads(request) or _extract_ticket_links(request.data):
                comment = EmployeeTicketComment.objects.create(
                    ticket=obj,
                    author=request.user,
                    author_employee=getattr(request.user, "employee_profile", None) if getattr(request.user, "is_authenticated", False) else None,
                    text="Shared files and links.",
                    links=_extract_ticket_links(request.data),
                )
                _save_ticket_attachments(obj, request, comment=comment)
                _notify_ticket_watchers(obj, request.user, "comment", comment.text, comment=comment)

            if status_changed_to_done and comment is None:
                _notify_ticket_watchers(obj, request.user, "status", "Marked as done. Please check it once.")

            if (status_value in (None, "", "null")) and not (isinstance(text, str) and text.strip()) and not _ticket_uploads(request) and not _extract_ticket_links(request.data):
                return Response(
                    {
                        "detail": "No writable fields for employee",
                        "allowed": ["status", "files", "links"] + comment_keys,
                    },
...
class EmployeeTicketNotificationsAPI(DebugForce200Mixin, APIView):
    disable_force_200 = True
    authentication_classes = [QuietJWTAuthentication, SessionAuthentication]
    renderer_classes = [JSONRenderer, BrowsableAPIRenderer]
    permission_classes = [IsAuthenticated]
    ...
    def get(self, request):
        qs = EmployeeTicketNotification.objects.select_related("ticket", "actor", "recipient_employee", "recipient_user").all()
        if _is_admin(request.user):
            qs = qs.filter(recipient_user=request.user)
        elif hasattr(request.user, "employee_profile"):
            qs = qs.filter(Q(recipient_user=request.user) | Q(recipient_employee=request.user.employee_profile))
        else:
            return Response({"detail": "Forbidden"}, status=status.HTTP_403_FORBIDDEN)

        if str(request.query_params.get("unread", "")).strip().lower() in {"1", "true", "yes"}:
        ...
        try:
            ...
            pass
        return Response([_notification_payload(n) for n in qs.order_by("-created_at")[:limit]], status=status.HTTP_200_OK)
    ...
    def post(self, request):
        ...
        if ids and not isinstance(ids, list):
            return Response({"detail": "ids must be a list or 'all'"}, status=status.HTTP_400_BAD_REQUEST)

        qs = EmployeeTicketNotification.objects.all()
        if _is_admin(request.user):
            qs = qs.filter(recipient_user=request.user)
        elif hasattr(request.user, "employee_profile"):
            qs = qs.filter(Q(recipient_user=request.user) | Q(recipient_employee=request.user.employee_profile))
...
class EmployeeTicketCommentsAPI(DebugForce200Mixin, APIView):
    ...
    def post(self, request, pk):
        ...
        has_files_or_links = bool(_ticket_uploads(request) or _extract_ticket_links(request.data))
        if not isinstance(text, str) or not text.strip():
            if not has_files_or_links:
                return Response({"detail": "text, files, or links are required"}, status=status.HTTP_400_BAD_REQUEST)
            text = "Shared files and links."
        ...
        _save_ticket_attachments(ticket, request, comment=comment)
        _notify_ticket_watchers(ticket, request.user, "mention" if _mentioned_labels(text) else "comment", text.strip(), comment=comment)
